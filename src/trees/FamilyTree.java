...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Invalid string, must include colon to separate parent and child");//with a useful message
		...
		else
		{
			...
			if(parentNode == null) {
				throw new TreeException("Data file is invalid, parent is not found");//If the parent node wasn't found, there must have been something wrong in the 
			}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("The node" + name1 + "does not exist"); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("The node" + name1 + "does not exist"); // Throw TreeException with a useful message
